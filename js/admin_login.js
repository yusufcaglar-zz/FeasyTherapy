...
function login() {
    if (!loginOperation) {
        ...
        if (!id_check || login_password.length < 8 || login_password.length > 16 || !upperCheck || !numberCheck) {
            ...
            var params = "operation=admin_login&id_number=" + encodeURIComponent(login_id) + "&password=" + encodeURIComponent(login_password) + "&phpsessid=" + phpsessid;
            var xhr = new XMLHttpRequest();
            xhr.open('POST', server_addr + 'php/Index.php', true);
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhr.send(params);
            ...
            xhr.onload = function () {
                ...
                if (server_response.result === "success")
                    parent.window.location.href = "page/admin.html";
