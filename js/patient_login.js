...
login_button.onclick = function () {
    var login_id = login_id_input.value;
    var login_password = login_password_input.value;
...
function login(login_id, login_password) {
    if (!loginOperation) {
        ...
        if (!id_check || login_password.length < 8 || login_password.length > 16 || !upperCheck || !numberCheck) {
            ...
            var params = "operation=patient_login&id_number=" + encodeURIComponent(login_id) + "&password=" + encodeURIComponent(login_password) + "&phpsessid=" + phpsessid;
            var xhr = new XMLHttpRequest();
            xhr.open('POST', server_addr + 'php/Index.php', true);
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhr.send(params);
            ...
            xhr.onload = function () {
                ...
                if (server_response.result === "success")
                    parent.window.location.href = "page/patient.html";
